...
describe('Tooltip组件测试', () => {
  ...
  describe('基本渲染', () => {
    ...
    it('应该根据位置设置样式', () => {
      const { container } = render(
        <Tooltip {...defaultProps} position={{ x: 50, y: 75 }} />
      )
  ...
  describe('内容格式化', () => {
    it('应该显示单个参数', () => {
      render(
        <Tooltip
          {...defaultProps}
          attributes={{ params: ['single-param'] }}
        />
      )
  ...
  describe('边界情况', () => {
    it('应该处理负数位置', () => {
      const { container } = render(
        <Tooltip {...defaultProps} position={{ x: -10, y: -20 }} />
      )
    ...
    it('应该处理零位置', () => {
      const { container } = render(
        <Tooltip {...defaultProps} position={{ x: 0, y: 0 }} />
      )
    ...
    it('应该处理大数值位置', () => {
      const { container } = render(
        <Tooltip {...defaultProps} position={{ x: 9999, y: 9999 }} />
      )
