...
export const useEditHistory = ({
  ...
  const saveToStorage = useCallback((
    ...
    newSpecialEntries: HistoryEntry[],
    newIndex: number
  ) => {
    try {
      const data = {
  ...
  const addHistoryEntry = useCallback((entry: HistoryEntry) => {
    ...
    if (isSpecial) {
      ...
      const merged = [...newSpecialEntries, ...entriesRef.current].sort((a, b) => b.timestamp - a.timestamp)
      ...
      // 普通版本：应用限制
      let newEntries = [...entriesRef.current, entry]
      ...
      const merged = [...specialEntriesRef.current, ...newEntries].sort((a, b) => b.timestamp - a.timestamp)
      const newIndex = merged.findIndex(e => e.id === entry.id)
   ...
   * 加载历史版本
   */
  const loadHistoryVersion = useCallback((index: number) => {
